...
# def companyNames():
#     COMPANYNAMES = ['CocaCola','Google','Meta','Elon Mush','Euphoria.io','Reddit','xkcd.com','@c+1']
#     return str(COMPANYNAMES[random.choice[0,1,2,3,4,5,6,7]])

# def pingMessage():
#         pingresponses = ['Fuck you, I was trying to sleep.','TFW @fumbleweed always beats me to the !ping?','/me angry bot noises','!ping','This ping response was sponsered by',companyNames]
#         n = random.choice[0,1,2,3,4]
#         return list(pingresponses)


COMPANY_NAMES = ['Google','Meta','CocaCola','Euphoria.io','c+1','Big Egg']
PING_responseS = ['Fuck you, I was trying to sleep.','TFW @fumbleweed always beats me to the !ping?','/me angry bot noises','This ping response was sponsored by Big Egg.','','monke','[reluctantly] pong.','I live in your walls.','I know what you did','no.',"[REDACTED]","Stop waking me up!","!tell @c+1 they're pinging me again! make them stop!","This ping response was sponsored by CocaCola","This ping response was sponsored by Google","WARNING: this bot is sponsored in part or in whole by the Chinese Government.","Ping response Option #22","I hate sand.","*plonk*","*plink*","*bonk*","Pong!",'https://imgur.com/ZkBZdi7']
numbers = ['1','2','2']

def pingMessage():
    nes = random.randint(0,len(COMPANY_NAMES))
...
class Bot(object):
    ...
    def handle_commands(self, info, message, do_ping=True, do_spec_ping=None,
        ...
        if cnt == '!ping':
            if do_ping:
                self.send_chat(pingMessage(), info['id'])
                return True
        elif check('ping'):
            ...
            if do_uptime:
                ts = time.time()
                self.send_chat('/me has been up for \"a while\" (%s (%s))' %
